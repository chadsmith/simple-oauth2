...
module.exports = function (config) {
  ...
  function create(token) {
    accessToken = token;
    accessToken.expires_at = (new Date).addSeconds(token.expires_in);
   ...
   */
  function expired() {
    return (Date.compare(accessToken.expires_at, new Date) === -1) ? true : false;
  }
  ...
  function revoke(tokenType, callback) {
    var token = tokenType === 'access_token' ? accessToken.access_token : accessToken.refresh_token;
